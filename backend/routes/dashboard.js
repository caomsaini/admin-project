...
const getCumulativeSummary = async () => {
  ...
  return lastEntry
    ...
    : [
        ...
        { title: "Total Orders Cancelled", value: 0 },
      ];
};
...
router.post("/realtime-update", async (req, res) => {
  try {
    ...
    // Update statusData and summaryData
    if (eventType === "newOrder") {
      data.statusData = updateStatusCardData(data.statusData, orderData);
      data.summaryData = updateSummaryCardData(data.summaryData, orderData);
    } else if (eventType === "statusChange") {
      data.statusData = updateStatusCardOnStatusChange(data.statusData, orderData);
      data.summaryData = updateSummaryCardOnStatusChange(data.summaryData, orderData);
    }
...
const updateSummaryCardData = (summaryData, orderData) => {
  return summaryData.map((item) => {
    ...
    } else if (item.title === "Total Units Sold") {
      const units = orderData.products.reduce((sum, p) => sum + p.quantity, 0);
      return { ...item, value: item.value + units };
    }
    return item;
...
const updateStatusCardOnStatusChange = (statusData, orderData) => {
  return statusData.map((item) => {
    if (item.title === orderData.previousStatus) {
      return { ...item, value: item.value - 1 };
    } else if (item.title === orderData.currentStatus) {
      return { ...item, value: item.value + 1 };
    }
    return item;
  });
...
const updateSummaryCardOnStatusChange = (summaryData, orderData) => {
  return summaryData.map((item) => {
    if (orderData.currentStatus === "Cancelled" && item.title === "Total Units Sold") {
      const units = orderData.products.reduce((sum, p) => sum + p.quantity, 0);
      return { ...item, value: item.value - units };
    } else if (orderData.currentStatus === "Cancelled" && item.title === "Total Sales") {
      return { ...item, value: item.value - orderData.totalPrice };
    }
    return item;
  });
